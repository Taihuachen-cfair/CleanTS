...
def evaluate_model_vis(
    ...
    *,
    ...
    for i, (inp, label) in enumerate(zip(test_data.input, test_data.label)):
        # 单条预测
        forecast = next(model.predict([inp]))
        ...
        # 保存为 PDF 文件（文件名可以自定义）
        pdf_path = f"/home/Data/chentaihua/pythonProject/uni2ts/pdf/forecast_plot_{i + 1}.pdf"
    ...
    # 最终得到完整 forecasts 迭代器
    forecasts = iter(forecasts_list)
