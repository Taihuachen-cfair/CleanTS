...
class PackCollate(Collate):
    ...
    max_samples_per_bin: int = float('inf')  # 新增参数，默认无限制
    ...
    def first_fit_decreasing_bin_packing(
        ...
        :return:
            ...
            - bin_spaces_array - length of each bin
        """
        batch = sorted(
        ...
        for sample in batch:
            length = len(sample[self.target_field])
            # Find the first bin that meets the following conditions:
            # 1) There is enough space
            # 2) The number of samples does not exceed the limit.
            placed = False
            for i in range(len(packed_batch)):
                if (
                        bin_spaces[i] >= length
                        and len(packed_batch[i]) < self.max_samples_per_bin
                ):
                    packed_batch[i].append(sample)
                    bin_spaces[i] -= length
                    placed = True
                    break
            if not placed:
                # new bin
                packed_batch.append([sample])
                bin_spaces.append(self.max_length - length)

        bin_spaces_array = np.array(bin_spaces, dtype=int)
        return packed_batch, bin_spaces_array
