...
def packed_attention_mask_VI(
    sample_id: Int[torch.Tensor, "*batch seq_len"],
    variate_id: Int[torch.Tensor, "*batch seq_len"],
) -> Bool[torch.Tensor, "*batch seq_len seq_len"]:
    """
    Compute attention mask that permits attention only within the same sample and same variate.
    Independent of variate values—uses only IDs to group positions.
    """
    sample_id = sample_id.unsqueeze(-1)
    variate_id = variate_id.unsqueeze(-1)

    sample_mask = sample_id.eq(sample_id.mT)      # same sample
    variate_mask = variate_id.eq(variate_id.mT)   # same variate

    attention_mask = sample_mask & variate_mask   # both must hold
